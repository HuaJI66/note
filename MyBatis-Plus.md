...
![img](imgs/relationship-with-mybatis.png)
...
<img src="imgs/mybatis-plus-framework.jpg" alt="framework" style="zoom:50%;" />
...
- **使用`Spring Initializer`快速初始化一个 Spring Boot 工程**
  ...
  <img src="imgs/image-20220519140839640.png" alt="image-20220519140839640" style="zoom:80%;" />

  <img src="imgs/image-20220519141335981.png" alt="image-20220519141335981" style="zoom:80%;" />

  <img src="imgs/image-20220519141737405.png" alt="image-20220519141737405" style="zoom:80%;" />

  <img src="imgs/image-20220519141849937.png" alt="image-20220519141849937" style="zoom:80%;" />
...
- **安装`Lombok`插件**
  ...
  <img src="imgs/image-20220519143257305.png" alt="image-20220519143257305" style="zoom:80%;" />
...
- **控制台打印查询结果**
  ...
  ![image-20220519150454211](imgs/image-20220519150454211.png)
...
- 我们将表`user`更名为`t_user`，测试查询功能
  ...
  ![image-20220520093844842](imgs/image-20220520093844842.png)

- 程序抛出异常，**Table 'mybatis_plus.user' doesn't exist**，因为现在的表名为`t_user`，而默认操作的表名和实体类型的类名一致，即`user`表

  ![image-20220520094126411](imgs/image-20220520094126411.png)
...
- 我们实体类中的属性`id`改为`uid`，将表中的字段`id`也改为`uid`，测试添加功能
  ...
  ![image-20220520100939157](imgs/image-20220520100939157.png)

  ![image-20220520100715109](imgs/image-20220520100715109.png)

- 程序抛出异常，**Field 'uid' doesn't have a default value**，说明MyBatis-Plus没有将`uid`作为主键赋值

  ![image-20220520101317761](imgs/image-20220520101317761.png)
...
![image-20220520103030977](imgs/image-20220520103030977.png)
...
- **数据库中创建逻辑删除状态列，设置默认值为0**
  ...
  ![image-20220520134529809](imgs/image-20220520134529809.png)

- **实体类中添加逻辑删除属性**

  ![image-20220520134636112](imgs/image-20220520134636112.png)
...
- **测试删除功能，真正执行的是修改**
  ...
  ![image-20220520135637388](imgs/image-20220520135637388.png)

- **此时执行查询方法，查询的结果为自动添加条件`is_deleted=0`**

  ![image-20220520140036445](imgs/image-20220520140036445.png)
...
![image-20220521092812125](imgs/image-20220521092812125.png)
...
- **条件的优先级**
  ...
  ![](imgs/image-20220923160254519.png)

  > **执行SQL：**
  >
  > UPDATE t_user SET is_deleted=1
  >  WHERE is_deleted=0 AND (user_name LIKE '%user%' AND age >= 19 OR email IS NOT NULL);
  ...
  ```java
      @Test
      public void testDel(){
          //删除(user_name中包含有user并且年龄大于等于19)或邮箱为null的用户
          QueryWrapper<User> queryWrapper = new QueryWrapper<>();
  ...
  ![](imgs/image-20220923160716413.png)

  

  执行前:

  ![image-20220923162224279](imgs/image-20220923162224279.png)

  > **执行SQL：**
  >
  > UPDATE t_user SET is_deleted=1
  >  WHERE is_deleted=0 AND (user_name LIKE '%user%' AND (age >= 19 AND email IS NOT NULL));

  

  ```java
      @Test
      public void testDelLambda() {
          //删除user_name中包含有user并且(年龄大于等于19或邮箱为null)的用户
          QueryWrapper<User> queryWrapper = new QueryWrapper<>();
          queryWrapper.like("user_name", "user")
                  .and(i -> i.ge("age", 19)
                          .isNotNull("email"));
  ...
  ![image-20220923162354867](imgs/image-20220923162354867.png)
...
>
  ...
  >  FROM t_user
  >  WHERE is_deleted=0 AND (age IN (select age
  >  FROM t_user
  >  WHERE age <= 20));
...
- **思路二**
  ...
  > SELECT uid AS id,user_name AS name,age,email,is_deleted
  >  FROM t_user
  >  WHERE is_deleted=0 AND (user_name LIKE '%null%' AND age > 19 AND email IS NOT NULL);
  ...
  ```java
      ...
      public void testCondition() {
        ...
        String email;
          ...
          QueryWrapper<User> queryWrapper = new QueryWrapper<>();
          queryWrapper.like("user_name", userName).gt("age", 19).isNotNull("email");
          userMapper.selectList(queryWrapper).forEach(System.out::println);
      }
  ```
  ...
  public void testCondition() {
      ...
      queryWrapper.like(StringUtils.isNotBlank(userName), "user_name", userName)
              .gt(age != null, "age", 19)
              .isNotNull(StringUtils.isNotBlank(email), "email");
      userMapper.selectList(queryWrapper).forEach(System.out::println);
  }
  ```
...
> SELECT uid AS id,user_name AS name,age,email,is_deleted
>  FROM t_user
>  WHERE is_deleted=0 AND (user_name LIKE '%test%' AND age > 19);

更换 user_name 字段名为 t_user_name:
...
> SELECT uid AS id,t_user_name AS name,age,email,is_deleted
>  FROM t_user
>  WHERE is_deleted=0 AND (t_user_name LIKE '%test%' AND age > 19);
...
```java
    ...
    public void testLambdaUpdate() {
        // 将用户名相似test并且年龄小于19岁的用户年龄修改为99
        LambdaUpdateWrapper<User> updateWrapper = new LambdaUpdateWrapper<>();

        updateWrapper.set(User::getAge, 99)
                .like(User::getName, "test")
                .and(x -> x.lt(User::getAge, 19));

        userMapper.update(null, updateWrapper);
    }
```

> UPDATE t_user SET age=99
>  WHERE is_deleted=0 AND (user_name LIKE '%test%' AND (age < 19));
...
- **执行结果**
  ...
  ![image-20220521225803162](imgs/image-20220521225803162.png)
...
- **优化执行流程**
  ...
  ![](imgs/image-20220521230448577.png)
...
- **数据库表添加字段`sex`**
  ...
  ![](imgs/image-20220521231317777.png)
...
public class FastAutoGeneratorTest {
    private static String url = "jdbc:mysql://localhost:13306/mybatis_plus?characterEncoding=utf-8&useSSL=false&serverTimezone=GMT%2B8";
    private static String userName = "root";
    private static String password = "root";

    public FastAutoGeneratorTest() {
    }

    public static void main(String[] args) {
        System.out.println("FastAutoGeneratorTest.url = " + FastAutoGeneratorTest.url);
        FastAutoGenerator.create(FastAutoGeneratorTest.url, FastAutoGeneratorTest.userName, FastAutoGeneratorTest.password)
                .globalConfig(builder -> {
                    builder.author("pika")  //设置作者
                            .fileOverride() //覆盖已生成文件
                            .outputDir("D://tmp/mybatis_plus"); //指定输出目录
                })
                .packageConfig(builder -> {
                    builder.parent("com.pika")  //设置父包名
                            .moduleName("mybatis_plus") //设置父包模块名
                            .pathInfo(Collections.singletonMap(OutputFile.mapperXml, "D://tmp/mybatis_plus"));  //设置mapperxml生成路径
                })
                .strategyConfig(builder -> {
                    builder.addInclude("t_user")    //设置要生成的表名
                            .addTablePrefix("t_");  //设置过滤器前缀
                })
...
spring:
  ...
  datasource:
    dynamic:
      # 设置默认的数据源或者数据源组,默认值即为master,需要与下面的'master'名字保持一致
      primary: master
...
![image-20220522113049945](imgs/image-20220522113049945.png)
...
![image-20220522115718361](imgs/image-20220522115718361.png)
...
- 在IDEA中与数据库建立链接
  ...
  ![image-20220522120758740](imgs/image-20220522120758740.png)

- 填写数据库信息并保存

  ![image-20220522121434468](imgs/image-20220522121434468.png)

- 找到我们需要生成的表点击右键

  ![image-20220522121613909](imgs/image-20220522121613909.png)

- 填写完信息以后下一步

  ![image-20220522122127649](imgs/image-20220522122127649.png)

- 继续填写信息

  ![image-20220522122525598](imgs/image-20220522122525598.png)

- **大功告成（真特么好用yyds）**

  ![image-20220522122612334](imgs/image-20220522122612334.png)
...
![image-20220522123143852](imgs/image-20220522123143852.png)

![image-20220522123202310](imgs/image-20220522123202310.png)
